...
static uint16_t roll, pitch, yaw, throttle;
...
void Bayang_init()
{
    ...
    NRF24L01_WriteReg(NRF24L01_07_STATUS, 0x70); // clear RX_DR, TX_DS, MAX_RT
    NRF24L01_FlushRx();
#endif
}
...
void Bayang_bind_rx()
{
    ...
    while(bind_count < 6) {
        ...
        while(millis()<timeout) {
            ...
            NRF24L01_ReadReg(NRF24L01_06_RF_SETUP);
            // NRF24L01_ReadReg(NRF24L01_05_RF_CH);
            readreg = NRF24L01_ReadReg(NRF24L01_07_STATUS);

            // if( readreg == 0x0e)
            // digitalWrite(ledPin, LOW);
            // Serial.println(readreg, HEX);
            if(readreg & _BV(NRF24L01_07_RX_DR)) { // data received from tx
                digitalWrite(ledPin, LOW);
...
//    }
    if (packet[0] == 0xA5)
    {
      ...
      if ( (sum&0xFF) == packet[14] )
      {
        ...
        // uint8_t packet[2], packet[3]
        // packet[2] | 0x00,

        //  packet[3] | 0x00

        uint8_t *sumd = BuildSumD(roll, pitch, yaw, throttle,
            packet[3] == BAYANG_FLAG_INVERT,
